...
export function FriendsAvatar({ name, avatarUrl, size = 'md' }: FriendsAvatarProps) {
  ...
  return (
    <span className={`${sizeClasses[size]} font-ui-title inline-flex items-center justify-center rounded-full border border-border bg-accent-light text-accent-strong shadow-sm`}>
      {initials || 'U'}
    </span>
  );
}
